<% @Page Language="VB" Explicit="True"%>
<%'======================================================================================
'=  CardinalCommerce (http://www.cardinalcommerce.com)
'=  ccRefund.aspx
'=  Version 6.0 03/21/2006
'=
'=  Usage
'=		Sample page to execute a refund transaction.
'======================================================================================%>
<!-- #Include File="CentinelConfig.aspx"-->
<%@ import Namespace="CardinalCommerce" %>
<%

Session("TID") = request("transaction_id")
Session("refundAmount") = request("refund_amount")
Session("currencyCode") = request("currency_code")

%>
<HTML>
<Head>
<Title>Refund Page</Title>
</Head>
<body>
<!--#Include  File = "ccMenu.aspx"-->
<br/>
<b>Refund Request Form</b>
<form name="frm" method="POST" action="ccRefund.aspx?execute=true">
<TABLE>
<TR bgcolor="ffff40">
	<TD>TransactionId </TD><TD><input type=text size="60" name="transaction_id" value="<%=Session("TID")%>"></TD>
</TR>
<TR bgcolor="ffff40">
	<td>Payment Type</TD><TD>
	<select name=txn_type>
		  <option value="C">C - Credit Card
		  <option value="P">P - PayPal
		  <option value="A">A - PayPal PreApproved Payments
  		  <option value="X">X - PayPal Express Checkout
		  <option value="E">E - Secure-eBill
		  <option value="B">B - Bill Me Later
	</select></TD>
</TR>
<TR bgcolor="ffff40">
	<TD>Refund Type</TD>
	<TD><select name=refund_type>
		<option value="Full">Full
		<option value="Partial">Partial
	</select></TD>
</TR>
<TR>
	<TD>Refund Amount </TD><TD><input type=text name="refund_amount" value="<%=Session("refundAmount")%>"></TD>
</TR>
<TR>
	<TD>Currency Code </TD>
    <TD><select name="currency_code">
          <option value="840">840 - USD</option>
          <option value="978">978 - EUR</option>
          <option value="392">392 - JPY</option>
          <option value="124">124 - CAD</option>
          <option value="826">826 - GBP</option>
		  <option value="036">036 - AUD</option>
        </select>
    </TD>
</TR>
<TR bgcolor="ffff40">
	<TD>Description </TD><TD><input type=text size="60" name="description" value="Refund Description"></TD>
</TR>
<TR>
  <TD></TD>
  <TD><input type=submit name="submit" value="Process Refund"></TD>
</TR>
<TR>
  <TD colspan="2"><br><b><i>Required fields highlighted</i></b></TD>
</TR>
</table>
</form>
<br>
<hr>
<br><br>
<%

	Dim executeTransaction
	executeTransaction = request("execute")

	if (executeTransaction = "true") then

		Dim strTermUrl, strCardEnrolled, strErrorNo, strErrorDesc, strStatusCode, strReasonCode, strTransactionId, strAcsUrl, strPAReq, strMerchantData, strTxnId
		Dim centinelRequest As New CentinelRequest()
		Dim centinelResponse As New CentinelResponse()

		'======================================================================================
		' Construct Message using Name / Value pairs
		'======================================================================================

		centinelRequest.Add ("Version", Cstr(MessageVersion))
		centinelRequest.Add ("MsgType", "cmpi_refund")
		centinelRequest.Add ("ProcessorId", Cstr(ProcessorId))
		centinelRequest.Add ("MerchantId", Cstr(MerchantId))
		centinelRequest.Add ("TransactionPwd", Cstr(TransactionPwd))
		centinelRequest.Add ("TransactionId", Cstr(request("transaction_id")))
		centinelRequest.Add ("TransactionType", Cstr(request("txn_type")))
		centinelRequest.Add ("RefundType", Cstr(request("refund_type")))
		centinelRequest.Add ("Amount", Cstr(request("refund_amount")))
		centinelRequest.Add ("CurrencyCode", Cstr(request("currency_code")))
		centinelRequest.Add ("Description", Cstr(request("description")))

		'=====================================================================================
		' Send the XML Msg to the MAPS Server
		' SendHTTP will send the cmpi_lookup message to the MAPS Server (requires fully qualified URL)
		' The Response is the CentinelResponse Object
		'=====================================================================================
		
		Try
			centinelResponse = centinelRequest.sendHTTP(Cstr(TransactionURL), Cstr(Timeout))
		
		Catch ex As Exception
           
			strErrorNo = "9040"
			strErrorDesc = "Communication Error"
		End Try

		If strErrorNo = "" Then
			
			strErrorNo = centinelResponse.getValue("ErrorNo")			 
			strErrorDesc = centinelResponse.getValue("ErrorDesc")
			strStatusCode = centinelResponse.getValue("StatusCode")
			strReasonCode = centinelResponse.getValue("ReasonCode")
			strTransactionId = centinelResponse.getValue("TransactionId")

		End If
		
		centinelResponse = nothing
		centinelRequest = nothing
%>


<b> Refund Transaction Request Results </b>
<TABLE>
<TR>
	<TD>ProcessorId</TD><TD><%=Cstr(ProcessorId)%></TD>
</TR>
<TR>
	<TD>MerchantId</TD><TD><%=Cstr(MerchantId)%></TD>
</TR>
<TR>
	<TD>TransactionPwd</TD><TD><%=Cstr(TransactionPwd)%></TD>
</TR>
<TR>
	<TD>TransactionId</TD><TD><%=request("transaction_id")%></TD>
</TR>
<TR>
	<TD>Refund Type</TD><TD><%=request("refund_type")%></TD>
</TR>
<TR>
	<TD>Refund Amount</TD><TD><%=request("refund_amount")%></TD>
</TR>
<TR>
	<TD>Currency Code</TD><TD><%=request("currency_code")%></TD>
</TR>
<TR>
	<TD>Description</TD><TD><%=request("description")%></TD>
</TR>
</TABLE>
<br/><br/>
<b> Refund Transaction Results </b>
<TABLE>
<TR>
	<TD>ErrorNo</TD><TD><%=strErrorNo%></TD>
</TR>
<TR>
	<TD>Description</TD><TD><%=strErrorDesc%></TD>
</TR>
<TR>
	<TD>Status Code</TD><TD><%=strStatusCode%></TD>
</TR>
<TR>
	<TD>Reason Code</TD><TD><%=strReasonCode%></TD>
</TR>
<TR>
	<TD>Transaction Id</TD><TD><%=strTransactionId%></TD>
</TR>
</TABLE>


<%

	End IF
%>


</body>
</html>

